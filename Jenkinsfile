...
//                sh "docker run -p 7001:7001 --name ${imageName} -d ${registry}/${imageName}:${version}"
                rancherRedeploy alwaysPull: true, credential: 'rancher', images: 'chjplove/kafka-producer-processor:latest', workload: '/p/c-b57jq:p-nn6qt/workload/deployment:default:kafka-producer-processor'
           }
           break;
   }
 } catch (e) {
